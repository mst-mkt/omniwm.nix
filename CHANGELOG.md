...
- `lib.monitorOverride` is split into `lib.monitor.{bar,dwindle,gap,niri,orientation,routing}`, one helper per settings list, so each entry is checked more strictly against its own list, including its required keys. `lib.routingArrangement` builds an element of `routing.arrangements` from `lib.monitor.routing` entries. `lib.monitorOverride` remains as deprecated. ([#19](https://github.com/mst-mkt/omniwm.nix/pull/19))
